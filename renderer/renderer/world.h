...
public:
	...
	world(const std::shared_ptr <camera>& cam, const std::vector<std::shared_ptr<triangle>>& list,const std::vector<std::shared_ptr<light>>& l)
	{
		object_list_ = list;
		camera_ = cam;
		sources_list_ = l;
...
namespace render
{
	...
	sf::Image Rasterizer(const int width, const int height, const std::vector<triangle>& triangles,const world& scene,bool shade = true)
	{
		...
		for (auto it = triangles.begin();it!=triangles.end();++it)
		{
			...
			if ((top_l_c[0] <= width) && (top_l_c[0] >= 0) && (bot_r_c[0] <= width) && (bot_r_c[0] >= 0)
				&& (top_l_c[1] <= height) && (top_l_c[1] >= 0) && (bot_r_c[1] <= height) && (bot_r_c[1] >= 0))
			{
					...
					for (int j = (int)(bot_r_c[1]); j < (int)top_l_c[1]; j++)
					{
						...
						if (utility::InsideTriangleTest(p, t_in))
						{
							...
							if ((double)z < (-((*scene.camera()).near()[2] + (*scene.camera()).center()[2])))
							{
								...
								{
									p_coords = utility::CoordsToSFML(p_coords, height,width);
									depthBuffer[j][i] = z;
		...
		for (int i = 0; i < height; i++)    //Free memory
